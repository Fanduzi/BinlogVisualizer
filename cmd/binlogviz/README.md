...
| `*_test.go` | Covers flag parsing, snapshot workflow behavior, compare registration and input validation, analyze/compare integration, temp-store cleanup behavior, discovery-mode input resolution, workflow run/resume end-to-end tests, workflow validate/describe/clean output contracts, workflow failure output that prints `Error:` once without Usage, and the GitHub release archive packing contract (binary + sample + bundled plan). |
...
- Analyze and workflow failure paths print the error once and do not dump full Usage. Empty/truncated/corrupt files use DBA-facing messages. A non-empty file with no Format Description exits non-zero; a time window with 0 matches prints `window matched 0 events` on `stderr`. HTML save confirmation goes to `stderr` only when a file was written. `analyze --format html` without `--output` writes a derived cwd file on a TTY and the HTML document to stdout when stdout is redirected. STATEMENT analyze (Query-DML with zero ROW images) exits 1 with empty stdout; `cobra.CheckErr` prints the STATEMENT/MIXED sentence once as `Error:` and no report is written. MIXED files that still have ROW images exit 0 and still write a report. The workflow command tree (`run`, `resume`, `validate`, `describe`, `status`, `clean`, `export`) sets `SilenceUsage` and `SilenceErrors` so `cobra.CheckErr` is the single `Error:` printer.
