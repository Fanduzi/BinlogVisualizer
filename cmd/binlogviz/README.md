...
- Analyze failure paths print the error once and do not dump full Usage. Empty/truncated/corrupt files use DBA-facing messages. A non-empty file with no Format Description exits non-zero; a time window with 0 matches prints `window matched 0 events` on `stderr`. HTML save confirmation goes to `stderr`. STATEMENT analyze (Query-DML with zero ROW images) exits 1 with empty stdout; `cobra.CheckErr` prints the STATEMENT/MIXED sentence once as `Error:` and no report is written. MIXED files that still have ROW images exit 0 and still write a report.
