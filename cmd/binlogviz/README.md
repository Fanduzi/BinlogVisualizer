...
- Successful MIXED analyze JSON (ROW images > 0 and ignored Query-DML > 0) appends one `alerts` entry `{type: input_format, severity: warning}` whose message states the guessed format, counted ROW images, and ignored Query-DML count. `warnings` stays the truncated-query integer. ROW with no Query-DML does not emit this alert. STATEMENT remains a non-zero exit without a success report.
