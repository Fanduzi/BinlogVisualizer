...
Cobra CLI entrypoints and command-layer orchestration for analyze, compare, trend, snapshot, version, and workflow commands including run, resume, validate, and describe.
...
| `workflow.go` | Implements `workflow run <plan.yaml>`, `workflow resume <output_dir>`, pre-run `workflow validate <plan.yaml>` checks, and static `workflow describe <plan.yaml>` previews. |
