...
| `analyze.go` | Orchestrates true streaming parse-normalize-consume-finalize execution, aggregate parse progress on `stderr`, report rendering, command-owned DuckDB temp-store lifecycle, and `--sql-context` presentation selection. |
