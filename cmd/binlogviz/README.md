...
| `analyze.go` | Orchestrates explicit-path or discovery-mode analyze execution through one streaming path (optional injected normalize, optional HTML dest), explicit `--workload-id` persistence, time/position/GTID selector flags, RFC3339 or local `YYYY-MM-DD HH:MM:SS` `--start`/`--end` parsing, documents UTC binlog timestamps and explicit-offset RFC3339 filters, selected-file coverage, unmapped parser-event and Ignored QUERY counts, Unclassified QUERY exit 1 with one `Error:` line when that QUERY intersected the window, after-window Unclassified QUERY keeping the in-window report, aggregate parse progress on `stderr`, Format Description server-version plumbing for replay commands, optional snapshot persistence, no-data exit 2 guards including ADMIN-only files, and command-owned DuckDB temp-store lifecycle. |
...
| `*_test.go` | Covers UTC/explicit-offset analyze help, local `YYYY-MM-DD HH:MM:SS` time parsing, flag parsing, inclusive-window adjacent-file planning, intersected time/position selection, GTID group filtering including zero-row XA COMMIT selection, report-v3 round trips, MySQL/MariaDB provenance and real `XAPrepareLogEvent`/LOAD_DATA SQL-context compatibility, snapshots, analyze/compare integration, Unclassified QUERY / Ignored QUERY / ADMIN-only command I/O including anonymous empty-identity next-GTID and finalize, after-window Unclassified QUERY exit 0 with the in-window report, real-parser exit 0 on the MySQL 8.0.46 `FLUSH TABLES` dialect fixture, cleanup, discovery/no-data behavior, workflows, and release packing; English golden output is pinned independently of the shell locale. |
...
- Command temp-store cleanup remains directory-scoped and no longer depends on any SQL-context sidecar file; bounded SQL context now lives entirely inside the analyzer's DuckDB temp DB. Workflow analyze uses the same default `--detail-store none` path as the analyze command; DuckDB is created only when that mode is requested and CGO is enabled.
...
- `analyze --help` states that binlog timestamps are UTC, that explicit RFC3339 offsets define the filter instant, and that `YYYY-MM-DD HH:MM:SS` is interpreted in the machine-local timezone of the running `binlogviz`; inclusive-window comparison and UTC report display are unchanged.
