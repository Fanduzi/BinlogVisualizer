...
- Analyze performance coverage includes text-vs-HTML rendering, corpus-backed DBA incident workloads, and a CI-safe near-1GB synthetic mix benchmark; real-fixture parser benchmarks isolate parse-only, parse+normalize, and end-to-end layers to identify per-stage bottlenecks; real 1GB binlog validation remains a manual DBA-environment gate.
