// Package binlogviz covers operator-facing analyze I/O: HTML destination, mysqlbinlog_cmd, exit codes, and empty-stdout no-data results.
// input: fixture binlogs, CLI args, and captured stdout/stderr from NewRootCommand.
// output: regression coverage for HTML redirect, copy-paste replay paths, magic-only exit 1, and window-miss/FD-only exit 2.
