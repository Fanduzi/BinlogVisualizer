...
| `transactions.go` | Reconstructs transaction groups, closes on COMMIT/XID/XA PREPARE/COMMIT/ROLLBACK, GTID-started DDL, and GTID-started ADMIN (ANALYZE TABLE / OPTIMIZE TABLE / FLUSH PRIVILEGES / SET DEFAULT ROLE / exact FLUSH TABLES) when there is no BEGIN, continues through XA END, fails Unclassified QUERY when that is the only work of a GTID-started non-explicit group, rejects conflicting GTIDs, names `retainCompletedTransaction` for report membership (ROW image rows, or XA identity with a file location), and uses a shared wrapper file span when expanded payload inners all carry that range. |
...
| `*_test.go` | Verifies intersected selection, complete/partial replay evidence, GTID flavor/precedence/rotation/DDL/XA/LOAD_DATA behavior, SCHEMA.TABLE object filtering, GRANT/CREATE USER DDL groups, ADMIN close via NormalizeRawEventInto plus Consume (not DDL, not zero-row report transactions, QUERY physical end, explicit BEGIN/XA still conflict), Unclassified QUERY fail vs Ignored QUERY conflicting GTID at the same seam, ParseFiles admission of exact FLUSH TABLES from the MySQL 8.0.46 dialect fixture, ParseFiles of the MySQL 8.0 compressed transaction-payload fixture including wrapper file-span once and position include/exclude of that span, anonymous GTID empty-identity omit under include-GTID, report retain of zero-row XA with a file location versus omit without one, XA END/ROLLBACK close rules, CGO DuckDB constructor contract, detail-store parity, and benchmarks. |
...
| `NewTransactionBuilder() *TransactionBuilder` | Reconstructs GTID-aware MySQL/MariaDB XA transaction boundaries, closes GTID-started non-explicit ADMIN groups, fails Unclassified QUERY when that is the only work of a GTID-started non-explicit group, fails on conflicting canonical GTIDs, and reports a shared wrapper file span when expanded payload inners all carry that range. |
| `UnclassifiedQueryError` | Analyze failure when a GTID-started non-explicit transaction group's only work is an Unclassified QUERY. `Error()` includes the bounded statement prefix and does not say conflicting GTID. |
