...
| `transactions.go` | Reconstructs transaction groups, closes on COMMIT/XID/XA PREPARE/COMMIT/ROLLBACK, GTID-started DDL, and GTID-started ADMIN (ANALYZE TABLE / OPTIMIZE TABLE / FLUSH PRIVILEGES / SET DEFAULT ROLE / exact FLUSH TABLES) when there is no BEGIN, continues through XA END, fails Unclassified QUERY when that is the only work of a GTID-started non-explicit group (named or anonymous empty identity; next GTID or finalize), rejects conflicting GTIDs, and names `retainCompletedTransaction` for report membership (ROW image rows, or XA identity with a file location). |
...
| `NewTransactionBuilder() *TransactionBuilder` | Reconstructs GTID-aware MySQL/MariaDB XA transaction boundaries, closes GTID-started non-explicit ADMIN groups, fails Unclassified QUERY when that is the only work of a GTID-started non-explicit group (named or anonymous empty identity), and fails on conflicting canonical GTIDs. |
