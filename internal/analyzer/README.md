...
| `*_test.go` | Verifies intersected selection, complete/partial replay evidence, GTID flavor/precedence/rotation/DDL/XA/LOAD_DATA behavior, SCHEMA.TABLE object filtering, GRANT/CREATE USER DDL groups, ADMIN close via NormalizeRawEventInto plus Consume (not DDL, not zero-row report transactions, QUERY physical end, explicit BEGIN/XA still conflict), Unclassified QUERY fail vs Ignored QUERY conflicting GTID at the same seam, ParseFiles admission of exact FLUSH TABLES from the MySQL 8.0.46 dialect fixture, ParseFiles of the MySQL 8.0 compressed transaction-payload fixture, anonymous GTID empty-identity omit under include-GTID, report retain of zero-row XA with a file location versus omit without one, XA END/ROLLBACK close rules, CGO DuckDB constructor contract, detail-store parity, and benchmarks. |
...
| `NewTransactionBuilder() *TransactionBuilder` | Reconstructs GTID-aware MySQL/MariaDB XA transaction boundaries, closes GTID-started non-explicit ADMIN groups, fails Unclassified QUERY when that is the only work of a GTID-started non-explicit group, and fails on conflicting canonical GTIDs. |
