...
| `*_test.go` | Verifies intersected selection, complete/partial replay evidence, GTID flavor/precedence/rotation/DDL/XA/LOAD_DATA behavior, SCHEMA.TABLE object filtering, GRANT/CREATE USER DDL groups, ADMIN close via NormalizeRawEventInto plus Consume (not DDL, not zero-row report transactions, QUERY physical end, explicit BEGIN/XA still conflict), Unclassified QUERY fail vs Ignored QUERY conflicting GTID at the same seam, ParseFiles admission of exact FLUSH TABLES from the MySQL 8.0.46 dialect fixture, report retain of zero-row XA with a file location versus omit without one, XA END/ROLLBACK close rules, CGO DuckDB constructor contract, detail-store parity, and benchmarks. |
