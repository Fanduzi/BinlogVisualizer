...
func TestTransactionBuilderSeparatesPreparedMariaDBXAFromFollowingGTID(t *testing.T) {
	...
	events := []model.NormalizedEvent{
		...
		{Timestamp: ts.Add(4 * time.Second), EventType: "GTID", GTID: "0-7-1860", BinlogPath: "mariadb-bin.000001", PositionStart: 4444, PositionEnd: 4500},
		...
		{Timestamp: ts.Add(5 * time.Second), EventType: "GTID", GTID: "0-7-1861", BinlogPath: "mariadb-bin.000001", PositionStart: 4560, PositionEnd: 4600},
	...
	if len(txns) != 3 {
		t.Fatalf("expected XA PREPARE, XA COMMIT, and following GTID as separate transactions, got %+v", txns)
	}
	...
	if txns[1].XAXID != xid || txns[1].GTID != "0-7-1860" || txns[1].TotalRows != 0 {
		t.Fatalf("unexpected XA COMMIT transaction: %+v", txns[1])
	}
	if txns[2].XAXID != "" || txns[2].GTID != "0-7-1861" || txns[2].TotalRows != 4 || txns[2].Tables["dogfood_cut.next_gtid"] != 4 {
		t.Fatalf("unexpected following GTID transaction: %+v", txns[2])
	}
}
