...
| `*_test.go` | Covers canonical event kinds, parser construction, helper behavior, transaction-payload expand, ParseFiles admission that compressed-payload inners share the wrapper file span once, normalization semantics including ADMIN (exact FLUSH TABLES, not FLUSH TABLES WITH READ LOCK), Unclassified QUERY emission, Ignored QUERY skip/count, and real-fixture parser benchmarks isolating parse-only, parse+normalize, and parse+progress layers. |
