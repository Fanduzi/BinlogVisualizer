	...
	if hasOffset {
		for index, path := range paths {
			...
			if probes[index].SizeBytes > 0 {
				last, err := probeLastTimestamp(offsetParser, path, probes[index].SizeBytes)
				if err != nil {
					return nil, fmt.Errorf("probe last timestamp %s: %w", path, err)
				}
				// If offset-based probe returned zero (e.g. seek landed mid-event with no valid events after),
