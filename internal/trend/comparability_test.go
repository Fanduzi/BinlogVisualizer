...
// input: complete report-v3 point/baseline series with explicit workload IDs, provenance, scopes, and raw numeric changes.
// output: regression coverage for all-point verdicts, baseline gating, renderer prominence, raw deltas, and suppression of ordinary trend narratives.
...
func TestGuardedTrendRenderersLeadWithEvidenceAndSuppressOrdinaryNarratives(t *testing.T) {
	...
	for name, render := range map[string]func(Result) (string, error){"text": RenderText, "html": RenderHTML} {
		t.Run(name, func(t *testing.T) {
			...
			for _, evidence := range []string{"test-workload", "other-workload", "mysql", "mariadb", "shop"} {
