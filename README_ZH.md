...
```bash
# install.sh（当前版本）
curl -fsSLO https://raw.githubusercontent.com/Fanduzi/BinlogVisualizer/v0.21.1/install.sh
sh ./install.sh --version v0.21.1

# 或 linux/amd64 tarball
curl -fsSLO https://github.com/Fanduzi/BinlogVisualizer/releases/download/v0.21.1/binlogviz_0.21.1_linux_amd64.tar.gz
tar -xzf binlogviz_0.21.1_linux_amd64.tar.gz
...
下面是 `darwin/arm64` 和当前版本 `v0.21.1` 的示例：

```bash
curl -fsSLO https://github.com/Fanduzi/BinlogVisualizer/releases/download/v0.21.1/binlogviz_0.21.1_darwin_arm64.tar.gz
curl -fsSLO https://github.com/Fanduzi/BinlogVisualizer/releases/download/v0.21.1/binlogviz_0.21.1_checksums.txt
shasum -a 256 -c binlogviz_0.21.1_checksums.txt 2>/dev/null | grep "binlogviz_0.21.1_darwin_arm64.tar.gz: OK"
tar -xzf binlogviz_0.21.1_darwin_arm64.tar.gz
install ./binlogviz /usr/local/bin/binlogviz
```
...
```bash
./install.sh --version v0.21.1 --dry-run
```
