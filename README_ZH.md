...
下面是 `darwin/arm64` + 当前 Phase 2 版本 `v0.2.2` 的示例：
...
```bash
curl -fsSL https://raw.githubusercontent.com/Fanduzi/BinlogVisualizer/main/install.sh | sh -s -- --version v0.2.2
```

仅预览解析到的 artifact，而不实际下载：

```bash
./install.sh --version v0.2.2 --dry-run
```
