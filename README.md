...
`compare` can load either two saved snapshots or two JSON files generated by `binlogviz analyze --format json`. It renders `text`, `json`, or `html` output. The HTML variant is a visual compare report with chart-based sections for summary deltas, top table changes, operation mix, and alert additions/removals.
...
The default snapshot store is `~/.binlogviz/snapshots`. Use `binlogviz snapshot save <report.json> --name <name>` when you already have an exported analyze JSON file and want to add it to that store later.
