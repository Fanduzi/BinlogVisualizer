...
`snapshot list --format json` and `snapshot show --format json` provide stable machine-readable output for scripts and external tooling. `snapshot rename` keeps the stored snapshot identity in sync with the renamed file, and `snapshot delete` removes one saved snapshot without touching the rest of the store.

`compare` can load either two saved snapshots or two JSON files generated by `binlogviz analyze --format json`. It renders `text`, `json`, or `html` output. The text and HTML variants now include snapshot context such as input mode, source summary, active filters, and requested window. The HTML variant is a visual compare report with chart-based sections for summary deltas, top table changes, operation mix, and alert additions/removals.
...
Example for `darwin/arm64` and the current release `v0.9.0`:
...
The default snapshot store is `~/.binlogviz/snapshots`. Use `binlogviz snapshot save <report.json> --name <name>` when you already have an exported analyze JSON file and want to add it to that store later. Use `snapshot rename` and `snapshot delete` to manage long-lived snapshot history without manual file operations.
