...
`compare` accepts exactly two positional arguments per invocation:

- `current.json`: the current BinlogViz analysis report
- `baseline.json`: the baseline BinlogViz analysis report

The command does not support discovery mode, binlog files, Markdown output, or extra input modes.

## `compare` Input Rules

`compare` only accepts JSON reports generated by `binlogviz analyze --format json`.
