...
- Normalize must emit `ANALYZE TABLE`, `OPTIMIZE TABLE`, `FLUSH PRIVILEGES`, and `SET DEFAULT ROLE` as `ADMIN`, not `DDL`. Silent skip leaves the GTID group open and the next GTID fails analyze (#72). Unknown QUERY, including other `SET` and `FLUSH` forms, stays skipped.
